...
更新(1-13)：注意多圈后左腿零点变化较大，可能需要写一个多圈解算之类（原自带编码器连续值不行），左腿旋转10圈后，原 $\theta$ 零点偏80度左右, 另外上电不能自启多数是临界变化阈值设置问题. ( 目前我是没有什么好的方法 )
...
R_mat = np.diag([500, 100])  # High 下蹲会旋转，s不好但是倒地可多次起
